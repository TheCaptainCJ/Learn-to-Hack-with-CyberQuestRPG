...
class App(ctk.CTk):
    ...
    def _build_rank_bar(self):
        ...
        ctk.CTkLabel(left, text=f"RANK {rank['level']}",
                     font=ctk.CTkFont("Courier New", 11), text_color=C["acc"]).pack(anchor="w")
        ...
        ctk.CTkLabel(right, text=f"{xp} XP",
        ...
        if next_rk:
            ...
            pb = ctk.CTkProgressBar(bar_row, progress_color=C["acc"], fg_color=C["acc3"], height=6)
            pb.pack(fill="x", pady=(0, 2))
            pb.set(min(progress, 1.0))
            ctk.CTkLabel(bar_row,
                         text=f"{rank['title']}  ->  {next_rk['title']}  ({next_rk['xp']-xp} XP to go)",
                         font=ctk.CTkFont("Courier New", 11), text_color=C["dim"]).pack(anchor="w")
        n_q = len(self.save.get("completed_quests", []))
        n_b = len(self.save.get("side_quests_done", []))
        ctk.CTkLabel(outer,
                     text=f"{n_q}/{len(ALL_QUESTS)} Quests  |  {n_b} Bosses Defeated",
                     font=ctk.CTkFont("Courier New", 11), text_color=C["dim"]).pack(side="bottom", anchor="w")
